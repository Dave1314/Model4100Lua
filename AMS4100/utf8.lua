...
--[[
This uses the _.upper array to convert a character to its
corresponding uppercase variant, if such exists.
--]]

function _.toupper(s)
    local t = ""
    for c in _.char(s) do
        if _.upper[c] then
            t = t .. _.utf8dec(_.upper[c])
        else
            t = t .. _.utf8dec(c)
        end
    end
...
--[[
This uses the _.lower array to convert a character to its
corresponding lowercase variant, if such exists.
--]]

function _.tolower(s)
    local t = ""
    for c in _.char(s) do
        if _.lower[c] then
            t = t .. _.utf8dec(_.lower[c])
        else
            t = t .. _.utf8dec(c)
        end
    end
